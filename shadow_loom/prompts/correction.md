...
| `set_belief_proposition_ids: [{entity_id, target_id, proposition_id}, ...]` | Backfill a `proposition_id` onto an existing belief (matched on `(entity_id, target_id)`). Use when validation flags a belief whose target maps to a catalogued proposition but `proposition_id` is null. |
...
| `duplicate_identity` (one character split into two `ENT_` ids — e.g. Anakin + Vader, Strider + Aragorn, Tom Riddle + Voldemort, Ben Kenobi + Obi-Wan) | `entity_renames: {minor_id: canonical_id}` merges them; beliefs and concerns from the merged-away entity are concatenated onto the survivor. **Detection cue:** if any entity's `name` field contains `" / "`, `"aka"`, `"(pre-fall)"`, `"(formerly ...)"`, or another parenthetical naming the same person under a second label — that's the `minor_id`, and the entry whose `name` is the dominant on-page form is the `canonical_id`. Same litmus also applies when `ENT_X` and `ENT_Y` share `location_id`, `status`, and overlap heavily in beliefs/concerns: very likely the same character. |
