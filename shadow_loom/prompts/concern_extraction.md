...
4. Pair ambivalent concerns explicitly — when you emit both a fear and a desire over the same proposition, populate each one's `counter_concern_ids` with the other's `concern_id`. Use the canonical per-entity pattern `CCN_{ENTITY_SUFFIX}_{TOPIC}` (e.g. `"CCN_LEAR_IRRELEVANCE"`, `"CCN_LEAR_LEGACY"`) — embedding the entity-id suffix in every concern id makes collisions across entities impossible by construction. Every `concern_id` MUST match the regex `^CCN_[A-Z0-9_]+$` and be unique within **this** entity's response. The downstream catalogue merge will detect and drop collisions across entities, but staying on the `CCN_{ENTITY}_*` convention avoids that overhead entirely.
