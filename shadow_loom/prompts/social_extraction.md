...
- `content` (str, optional): The actual prose / paraphrase of what was said. **Must be the in-world speech act verbatim (or a tight paraphrase of it).** NEVER append meta-commentary about the utterance's truth status — phrases like *"…, which is false"*, *"…, but he was lying"*, *"…, although the audience knows otherwise"* belong in `description`, never in `content`. The truth flag is the separate `truth_value` field; the renderer and downstream auditors rely on `content` being a clean reproduction of what the speaker actually said so they can faithfully voice the character without leaking author-omniscient framing into the prose. Bad: `"I testify that George committed the crime, which is false."` Good: `content="I testify that George committed the crime."`, `truth_value="false"`, `description="Wanda perjures herself on the stand."`.
...
- `at_location_id` (str | null): The `LOC_` ID where the utterance physically takes place. **Required for face-to-face utterances** (any utterance with `via_channel_id=null` AND non-empty `addressee_ids`) — name the room / setting where the speaker and addressees are co-present. **Leave null only when** `via_channel_id` is set (the channel handles transmission across distance) or the utterance has no addressees (a soliloquy with no audience). The auditor enforces co-presence: a face-to-face utterance whose addressees are not at `at_location_id` at `fabula_time` will be flagged.
