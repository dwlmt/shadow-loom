...
2. **One person = one entity, even across identity reveals or transformations.** When a single character is known by different names at different points in the story (a hidden identity revealed mid-narrative, an alias, a code name, a pre- and post-transformation name like Anakin → Vader, Strider → Aragorn, Tom Marvolo Riddle → Voldemort, Eleanor → Mrs. Dashwood-after-marriage), emit ONE `ENT_` whose canonical `name` is the most narratively-prominent form, and list every other form in `aliases` (or in the `name` parenthetical if no aliases field exists). DO NOT create two separate entities for the same person — that produces a `duplicate_identity` validation error and the only fix is a destructive re-extraction (the patch schema has no entity-merge capability). Litmus test: if the same physical body / consciousness is involved, it's one entity. If the text reveals on page 200 that "X was actually Y all along", that's still ONE entity, registered under whichever name dominates the prose.
...
7. **Belief target_id**: Must reference `ENT_`, `OBJ_`, or `LOC_` IDs from the registers provided, or other `ENT_` IDs you are extracting in this pass. Do NOT invent `EVT_` IDs (events have not been extracted yet) and do NOT use `WORLD_` IDs (world traits are extracted in parallel).
...
9. **Entities are agents, not places.**
