...
- `inverse_proposition_id` (str | null, optional): The PROP_ id whose truth value is the *logical opposite* of this one (`PROP_DUNCAN_DEAD` ↔ `PROP_DUNCAN_ALIVE`). When set, a truth commit on either proposition is automatically mirrored onto the inverse at the same fabula tick by the Phase C truth-write sweep, keeping the pair in lockstep. Use this only when the two propositions are genuinely contradictory across the whole story (a binary status like alive/dead, married/single, monarch/deposed); do NOT link merely correlated propositions. Leave null when no inverse exists in the catalogue. Both members of the pair should cross-reference each other.
