...
> - `mechanism` should be one of: `physical`, `psychological`, `epistemic`, `social`, `emotional`, `informational`, `betrayal`. Long-form aliases (`physical_force`, `epistemic_revelation`, `social_coercion`) are auto-canonicalized to the short form. Off-list labels (`kinetic`, `chemical`, `seduction`, `coercion`, `deduction`) are tolerated as an **escape-hatch only** — they bypass mechanism→trait routing entirely (full weight to every trait, no 20% fallback penalty), which is almost always wrong for narrative causation. **Default to canonical labels.** Reach for off-list ones only when you have an explicit reason to want uniform application across all of the target's traits (a god-tier curse, a world-ending shock, a metaphysical reset) AND no canonical label fits. If your trait_target is simply not in your chosen mechanism's routed list, change the mechanism, not the label.
...
- `mechanism` (str): How the cause produced the effect. Use one of these canonical values when possible:
  ...
  When the trait you target is **not** in the routed list for the chosen mechanism, the engine applies a fallback factor of `0.2` to the impulse (so the edge contributes only 20% of nominal weight to that trait). **The correct response is almost always to switch mechanism, not to escape the gate.** Off-list labels (`"kinetic"`, `"chemical"`, `"seduction"`, `"coercion"`, `"deduction"`) bypass routing entirely (full weight, all traits) and exist as an escape hatch for rare god-tier / metaphysical / world-ending impulses where uniform application is genuinely desired. They are not a convenience for ordinary edges whose trait_target doesn't fit — reach for them only when no canonical mechanism applies. Note that `"betrayal"` is **canonical**, not an escape hatch — it routes onto `anger`/`grief`/`fear`/`loyalty`/`affinity` like the other seven canonical mechanisms. For mutation edges, the safest pattern is: pick the canonical mechanism whose routed traits already include your `trait_target`.
- `necessity` (str, optional, default `"sufficient"`): The INUS-style modality of this causal contribution (Mackie 1965; Halpern–Pearl actual-causation). Drives disjunctive closure under do-prevention surgery. One of:
  - `"sufficient"` (default) — this parent on its own is enough to produce the effect. Under disjunctive closure, the descendant survives as long as *at least one* sufficient parent survives. Use for true sufficient causes (the persuasion that drives the murder; the spark that lights the powder; the order that triggers the execution).
  - `"necessary"` — the effect cannot occur without this parent, regardless of how many sufficient parents survive. Pruning a `necessary` parent forces the descendant into the prevented closure. Use for true preconditions (the victim must be co-located with the killer; the witness must be alive to testify; the seal must be unbroken for the curse to bind; the inheritance must be unsigned for the dispute to arise).
  - `"contributory"` — this parent shifts probability but is neither sufficient on its own nor strictly required. Treated as `sufficient`-with-weakened-confidence by the closure. Use for circumstantial contributors (the rainy weather that made the road slick; the rumour that softened the public mood; the lieutenant's hesitation that widened the window). When in doubt, leave it `"sufficient"` — mis-tagging `"necessary"` makes the counterfactual prune cascades larger than the story justifies.
  Worked examples: a regicide event has `necessity="sufficient"` for `EVT_DUNCAN_DIES` (Macbeth alone suffices); the witches' prophecy `EVT_WITCHES_HAIL` has `necessity="contributory"` (it tilts probability, doesn't compel); the king's solitary unguarded chamber affordance has `necessity="necessary"` for `EVT_DUNCAN_MURDER` (no opportunity, no murder).
...
7. **Per-`event_type` field requirements** — fill the right slots for the right kind of event, otherwise the auditor will reject the beat:
   ...
   - **`event_type="revelation"` (narrator-side disclosure to the reader)** — `actor_ids` MUST be empty `[]`. Revelations have no in-world actor; the narrator is not an entity in the storyworld. If a *character* tells another character the secret, that is an utterance and belongs to Step 3b — do NOT emit it here. `content` REQUIRED (the proposition the narration discloses, e.g. `"Snape was loyal to Dumbledore all along"`). `asserts_proposition_id` or `denies_proposition_id` REQUIRED if a matching `PROP_` exists in the catalogue; otherwise leave both null. `truth_value` REQUIRED — almost always `"true"` for narrator reveals (the narrator is canonically reliable unless the work is explicitly an unreliable-narrator frame). `target_ids` is the set of `EVT_`/`ENT_`/`OBJ_` ids the revelation is *about* (the referents of `content`), not its downstream effects.
     ...
     **The English word "reveals" is NOT a marker of `event_type="revelation"`.** Source phrasings like *"Analysis of the Death Star schematics reveals a weakness in the exhaust port"*, *"the autopsy reveals traces of poison"*, or *"the search reveals the hidden room"* describe a *fabula-side outcome event*: an in-world agent (rebel analysts, the coroner, the searcher) performed an action and obtained a finding. These are `event_type="outcome"` with `actor_ids=[<who-did-the-analysis>]` and `target_ids=[<what-was-analysed>]`. Reserve `revelation` strictly for *narrator-to-reader* moments where there is no in-world agent doing the disclosing — chapter-end twists, omniscient-narrator asides (*"Reader, I married him"*), unmasked-killer reveals where the prose itself lifts the veil.
