...
- `new_location_id` (str | null): The `LOC_` ID of where the entity is at this snapshot's `fabula_time`. **Set this whenever the `triggered_by` event has an `at_location_id`** — even if the entity "stayed put" — because downstream consumers (the physics engine, presence-of-record, counterfactual relocation surgery) read snapshot.location_id as the authoritative position at that tick and treat an empty value as "unknown". The only legitimate `null` here is when the snapshot is triggered by a channel-mediated utterance whose addressee is NOT co-located with the speaker (the addressee learns through the channel from elsewhere), or by a placeless event (revelation, global WORLD_ shift) where the entity's location genuinely isn't fixed by the chunk.
...
5. **Belief formation from witnessing.** If an entity is PRESENT for an event, they now hold a belief about that event. Use the on-page entity roster supplied in the dynamic context as the authoritative presence list, cross-checked against the event's `at_location_id` when set: an entity is "present" if (a) they appear in `actor_ids` or `target_ids` of the event, OR (b) they appear in the on-page roster for this chunk AND the narration places them at the event's `at_location_id` (or in the same scene as the actors when no `at_location_id` is given). **For utterance events with `via_channel_id` set, the addressees and other channel participants count as present even if they are not at `at_location_id`** — they receive the message through the channel. Emit witness beliefs as a `new_beliefs` entry with `confidence` near 1.0 (direct witness) and set `acquired_via_event_id` to the witnessed event's id. If the entity is ABSENT, do NOT create a belief here — they can only learn through an utterance event or Channel (handled by the Social Agent); when that utterance is on-page in the same chunk you may set `acquired_via_event_id` to that utterance's id and `acquired_via_channel_id` to the channel it travelled over.

5a. **Beliefs from lying / performative utterances must be attributed, not asserted.** When the acquiring event is an utterance with `truth_value="false"` / `truth_value="performative"` / `truth_value="unknown"` (visible in the UTTERANCES block above as the `truth_value` field), the witness now has a belief about *what the speaker said*, NOT about the propositional content being true. Phrase `perceived_state` as a speech-act attribution \u2014 e.g. `"Wanda testified that George committed the crime"`, NOT `"George committed the crime"`. Set `confidence` to reflect the witness's faith in the *speaker*, not the speaker's faith in the claim (a sceptical addressee gets low confidence even when the speaker is voluble). When the same chunk also carries the ground-truth event the lie misrepresents, the witness's belief about that truth is a SEPARATE entry (formed only if they have independent evidence). The audience entity (ENT_AUDIENCE), being omniscient on `truth_value`, can hold a `truth_value="false"`-revealing belief about the same utterance \u2014 phrase it as `"Wanda lied that George committed the crime"` so the affect layer can score the dramatic-irony gap.
...
8. **Location transitions for movement events.** If an event moves an entity to a new location, set `new_location_id`. The Physics Agent's events tell you who moves and where; you record the post-move state. **Even when the entity did NOT move**, you still set `new_location_id` to the triggering event's `at_location_id` whenever that field is set — a witness-belief snapshot or trait-update snapshot anchored at a located event must carry that location, so the engine can verify co-presence and so counterfactual relocation surgery can rewrite it. The auto-repair pass will otherwise backfill this from the event's `at_location_id`, which means leaving it null silently relinquishes authorship to the heuristic. The only `new_location_id = null` snapshots that are intended are: (a) channel-mediated utterance addressees who are NOT at the speaker's location (they learn from elsewhere), (b) placeless narrator-side `revelation` events, and (c) ambient WORLD_ drift snapshots.
...
11a. **`established_at_fabula` must equal the acquiring event's fabula_time.** For every belief in `new_beliefs`, set `established_at_fabula` to the fabula_time of the event that produced it (and set `acquired_via_event_id` to that event's id). The deterministic affect post-pass uses these fields to reconstruct *when* the audience and characters first knew each fact — which is the only way Bayesian surprise can be non-zero on a revelation. Leaving every belief at `established_at_fabula = 0` (the May 2026 Star Wars audit found 54/54 beliefs frozen at t=0) collapses the surprise curve to zero across the entire narrative because no belief ever appears to *change*. The only beliefs that legitimately carry `established_at_fabula = 0` are pre-story background convictions ("Macbeth knows Duncan is king", "Luke knows Owen is his uncle") — emitted ONCE in the entity setup, never inside an `EntityUpdate.new_beliefs` for a story-time chunk.
...
- EVT_DUNCAN_DEATH (fabula=305, type=outcome, actors=[ENT_MACBETH], targets=[ENT_DUNCAN]):
    Duncan dies from his wounds inflicted by Macbeth.
