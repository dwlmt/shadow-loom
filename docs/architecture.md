...
| **Surprise** | Per-trait binary KL divergence $D_\text{KL}(p \| q) = p\log\tfrac{p}{q} + (1-p)\log\tfrac{1-p}{1-q}$. Posterior $p$ is the entity's *final-state* trait value resolved via `reconstruct_entity_at(ent, t_max)` so authored `state_timeline` arcs are honoured (sandbox-preferred when running counterfactuals). Prior $q$ starts at the **leave-one-out** per-trait corpus marginal (mean across every *other* entity, falling back to 0.5 when fewer than two other entities carry the trait) — leave-one-out prevents the focal entity from biasing its own prior, which would otherwise collapse KL on the small casts typical of the example fixtures. The prior is then refined by a **Beta-Bernoulli** update (replacing the legacy geometric pull): the corpus marginal $m$ seeds a $\text{Beta}(s\,m,\, s(1-m))$ prior (pseudo-count strength $s = 2$), and each revealed causal edge targeting the entity contributes Bernoulli evidence with weight $w$ — $\alpha \mathrel{+}= w\cdot\text{actual}$, $\beta \mathrel{+}= w\cdot(1-\text{actual})$ — with the posterior mean $\alpha/(\alpha+\beta)$ returned (the same Bayesian core the EFK suspense scorer uses, so surprise and suspense share one update rule). Each per-trait KL is run through a soft-saturation $1 - e^{-\text{KL}}$ (so perceptually meaningful KLs in the 0.2–1.5 band map to 0.18–0.78 of the gauge) and the result is averaged across the focal traits. This trait-KL term is then combined with the **anachrony** component ($w_t=0.7$, $w_a=0.3$ convex pair) and an **additive audience-belief-KL booster** ($w_b=0.4$), clamped to $[0,1]$. The earlier $\text{avg}(\text{KL})/\log(1/\varepsilon)$ form divided by the *theoretical* binary-KL maximum ($\approx 4.6$ at $\varepsilon = 0.01$), squashing the entire perceptual signal into the bottom 4% of the gauge — every ``example_world`` plot read as flat ≤ 0.10 even when canonical surprise traits (Macbeth's despair, Macduff's grief) carried per-trait KLs of 0.27–0.50. See [academic-foundations.md §3.3](academic-foundations.md#33-surprise-as-kl-divergence). |
| **Tension** | Weighted composite of the other structural-affect channels: $T = 0.40\cdot\text{suspense} + 0.25\cdot\text{mystery} + 0.20\cdot\text{irony} + 0.10\cdot|\Delta\text{surprise}| + 0.05\cdot\text{unpaid-setup-debt}$, clamped to $[0,1]$. Suspense dominates (Brewer & Lichtenstein); mystery and irony add their current cumulative gaps; surprise contributes its *first derivative* (local form) as the Friston-2010 prediction-error kick; unpaid setup debt adds the Chekhov's-gun overhang from foreshadowing arcs. Implemented in `DirectiveAssembler.compute_tension_score`. |
...
* **Syuzhet axis** — the reader's progress through the *told* order.
  ...
  *Cumulative surprise* can therefore only move toward zero (the prior's
  Beta-Bernoulli posterior never overshoots the truth); *local surprise*
