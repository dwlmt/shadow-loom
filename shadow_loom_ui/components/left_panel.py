...
def _render_explorer(state: AppState, container) -> None:
    ...
    if ws is None:
        with container:
            ui.label("No world model.").classes("text-xs text-slate-500")
        return
...
def _render_inspector(state: AppState, container, node_id: str | None, node_type: str | None) -> None:
    ...
    if ws is None or node_id is None:
        with container:
            ui.label("Click a node to inspect.").classes("text-xs text-slate-500 p-2")
        return
    ...
    with container:
        if node_type == "Entity" and node_id in ws.entities:
            ...
            ui.label(f"Unknown: {node_id}").classes("text-xs text-slate-500 p-2")


def _inspect_entity(ws: WorldStateV1, eid: str) -> None:
    ent = ws.entities[eid]
    ui.label(ent.name).classes("text-base font-semibold text-slate-800 p-1")
    with ui.row().classes("gap-1 q-px-xs"):
        ...
        if ent.location_id in ws.locations:
            ui.badge(f"@ {ws.locations[ent.location_id].name}", color="secondary").props("dense")
    ...
    # Constants
    if ent.constants:
        ui.label("Constants").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for c in ent.constants:
            ui.label(f"• {c}").classes("text-xs text-slate-600 px-2")

    # Beliefs
    if ent.beliefs:
        ui.label("Beliefs").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
    ...
    if ent.state_timeline:
        ui.label("Recent State Changes").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for snap in ent.state_timeline[-5:]:
            ...
            ui.label(" | ".join(parts)).classes("text-xs text-slate-600 px-2")


def _inspect_location(ws: WorldStateV1, lid: str) -> None:
    loc = ws.locations[lid]
    ui.label(loc.name).classes("text-base font-semibold text-slate-800 p-1")
    if loc.description:
        ui.label(loc.description[:120]).classes("text-xs text-slate-500 px-1")

    # Entities here
    here = [e.name for eid, e in ws.entities.items() if e.location_id == lid]
    if here:
        ui.label("Entities Present").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for name in here:
            ui.label(f"• {name}").classes("text-xs text-slate-600 px-2")

    # Ambient state
    if loc.ambient_state:
        ui.label("Ambient State").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for k, v in loc.ambient_state.items():
            ui.label(f"{k}: {v.value:.2f}").classes("text-xs text-slate-600 px-2")
    ...
    if connections:
        ui.label("Connections").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for se in connections:
            ...
            ui.label(f"↔ {other_name}{lock}").classes("text-xs text-slate-600 px-2")


def _inspect_event(ws: WorldStateV1, evt) -> None:
    ui.label(evt.id).classes("text-base font-semibold text-slate-800 p-1")
    with ui.row().classes("gap-1 q-px-xs"):
    ...
    ui.label(evt.description).classes("text-xs text-slate-600 px-1 mt-1")

    if evt.actor_ids:
        names = [ws.entities[a].name if a in ws.entities else a for a in evt.actor_ids]
        ui.label(f"Actors: {', '.join(names)}").classes("text-xs text-slate-600 px-1")
    if evt.target_ids:
        names = [ws.entities[t].name if t in ws.entities else t for t in evt.target_ids]
        ui.label(f"Targets: {', '.join(names)}").classes("text-xs text-slate-600 px-1")
    ...
    if causes:
        ui.label("Caused by").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for ce in causes:
            ui.label(f"← {ce.source_id} ({ce.mechanism})").classes("text-xs text-slate-600 px-2")
    if effects:
        ui.label("Causes").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for ce in effects:
            ui.label(f"→ {ce.target_id} ({ce.mechanism})").classes("text-xs text-slate-600 px-2")


def _inspect_object(ws: WorldStateV1, oid: str) -> None:
    obj = ws.objects[oid]
    ui.label(obj.name).classes("text-base font-semibold text-slate-800 p-1")
    if obj.owner_id and obj.owner_id in ws.entities:
        ui.label(f"Owner: {ws.entities[obj.owner_id].name}").classes("text-xs text-slate-600 px-1")
    if obj.location_id and obj.location_id in ws.locations:
        ui.label(f"Location: {ws.locations[obj.location_id].name}").classes("text-xs text-slate-600 px-1")
    if obj.properties:
        ui.label("Properties").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for k, v in obj.properties.items():
            ui.label(f"{k}: {v}").classes("text-xs text-slate-600 px-2")
    if obj.affordances:
        ui.label("Affordances").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
        for a in obj.affordances:
            ui.label(f"• {a.action} → {a.target_type}").classes("text-xs text-slate-600 px-2")


def _inspect_world_trait(ws: WorldStateV1, wid: str) -> None:
    wt = ws.world_traits[wid]
    ui.label(wt.name).classes("text-base font-semibold text-slate-800 p-1")
    if wt.description:
        ui.label(wt.description[:120]).classes("text-xs text-slate-500 px-1")
    ui.label(f"Magnitude: {wt.magnitude.value:.2f} (inertia: {wt.magnitude.inertia:.2f})").classes(
        "text-xs text-slate-600 px-1"
    )
    if wt.affected_domains:
        ui.label(f"Domains: {', '.join(wt.affected_domains)}").classes("text-xs text-slate-600 px-1")
    if wt.state_timeline:
        ui.label("Timeline").classes("text-xs font-semibold text-slate-700 mt-2 px-1")
...
def _inspector_suggestions(state: AppState, node_id: str, node_type: str, name: str) -> None:
    ...
    if suggestions:
        ...
        ui.label("Try:").classes("text-xs text-slate-500 px-1")
