...
world_state = WorldStateV1(
    ...
    events=[
        EventNode(id="EVT_JOBS_LOST", fabula_time=1, syuzhet_index=1, event_type="outcome", actor_ids=[], description="Nick and Amy both lose their writing jobs in the 2009 recession."),
        EventNode(id="EVT_TRUST_FUND_DEPLETED", fabula_time=2, syuzhet_index=2, event_type="outcome", actor_ids=[], description="Amy's parents ask for money from her trust fund due to financial troubles; Nick uses the remainder to open a bar with Go."),
        EventNode(id="EVT_RELOCATE_MISSOURI", fabula_time=3, syuzhet_index=3, event_type="choice", actor_ids=["ENT_NICK"], description="Nick and Amy relocate to North Carthage, Missouri to care for Nick's sick mother."),
        EventNode(id="EVT_MARRIAGE_DETERIORATES", fabula_time=4, syuzhet_index=4, event_type="outcome", actor_ids=[], description="Their marriage deteriorates. Amy resents suburban life; Nick begins an affair with student Andie."),
        EventNode(id="EVT_AMY_DISCOVERS_AFFAIR", fabula_time=5, syuzhet_index=5, event_type="revelation", actor_ids=["ENT_AMY"], description="Amy discovers Nick's affair and begins planning an elaborate revenge over the next year."),
        EventNode(id="EVT_AMY_DISAPPEARS", fabula_time=6, syuzhet_index=6, event_type="choice", actor_ids=["ENT_AMY"], description="On their fifth anniversary, Amy stages her disappearance with faked signs of struggle, planting evidence to frame Nick."),
        EventNode(id="EVT_NICK_SUSPECT", fabula_time=7, syuzhet_index=7, event_type="outcome", actor_ids=[], description="Nick becomes the prime suspect. Media scrutiny, faked pregnancy, credit card debt, and increased life insurance all point to him."),
        EventNode(id="EVT_NICK_FINDS_CLUES", fabula_time=8, syuzhet_index=8, event_type="revelation", actor_ids=["ENT_NICK"], description="Nick follows Amy's treasure hunt clues and discovers the woodshed full of incriminating evidence. He realizes Amy framed him."),
        EventNode(id="EVT_AMY_ROBBED", fabula_time=9, syuzhet_index=9, event_type="outcome", actor_ids=[], description="Amy is robbed at the motel hideout, losing her cash reserves."),
        EventNode(id="EVT_AMY_SEEKS_DESI", fabula_time=10, syuzhet_index=10, event_type="choice", actor_ids=["ENT_AMY"], description="Desperate, Amy contacts her wealthy ex Desi Collings, who hides her in his lake house."),
        EventNode(id="EVT_NICK_TV_INTERVIEW", fabula_time=11, syuzhet_index=11, event_type="choice", actor_ids=["ENT_NICK"], description="With Tanner Bolt's help, Nick gives a TV interview performing the perfect apologetic husband."),
        EventNode(id="EVT_NICK_ARRESTED", fabula_time=12, syuzhet_index=12, event_type="outcome", actor_ids=[], description="Police discover the woodshed and Amy's faked diary. Nick is arrested."),
        EventNode(id="EVT_AMY_WATCHES_INTERVIEW", fabula_time=13, syuzhet_index=13, event_type="revelation", actor_ids=["ENT_AMY"], description="Amy sees Nick's TV interview and is impressed, convinced they are uniquely matched."),
        EventNode(id="EVT_DESI_MURDERED", fabula_time=14, syuzhet_index=14, event_type="choice", actor_ids=["ENT_AMY"], target_ids=["ENT_DESI"], description="Amy mutilates herself to fake captivity, seduces Desi, then murders him to escape and frame him as her kidnapper."),
        EventNode(id="EVT_AMY_RETURNS", fabula_time=15, syuzhet_index=15, event_type="outcome", actor_ids=["ENT_AMY"], description="Amy returns to North Carthage with a fabricated kidnapping story. Nick is released."),
        EventNode(id="EVT_FORCED_MARRIAGE", fabula_time=16, syuzhet_index=16, event_type="outcome", actor_ids=[], description="Nick, Go, and Boney know Amy is lying but have no proof. Nick is forced back into married life."),
        EventNode(id="EVT_AMY_INSEMINATES", fabula_time=17, syuzhet_index=17, event_type="choice", actor_ids=["ENT_AMY"], description="Amy uses Nick's frozen semen to become pregnant, using the unborn child as leverage to keep Nick compliant."),
        EventNode(id="EVT_NICK_SUBMITS", fabula_time=18, syuzhet_index=18, event_type="outcome", actor_ids=["ENT_NICK"], description="Amy forces Nick to delete his exposé. Nick dedicates himself to the role of perfect husband."),
    ],

    # ── CAUSAL TOPOLOGY ────────────────────────────────────────────────────
    causal_topology=[
        CausalEdge(source_id="EVT_JOBS_LOST", target_id="EVT_TRUST_FUND_DEPLETED", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=1),
        CausalEdge(source_id="EVT_TRUST_FUND_DEPLETED", target_id="EVT_RELOCATE_MISSOURI", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=2),
        CausalEdge(source_id="EVT_RELOCATE_MISSOURI", target_id="EVT_MARRIAGE_DETERIORATES", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=3),
        CausalEdge(source_id="EVT_MARRIAGE_DETERIORATES", target_id="EVT_AMY_DISCOVERS_AFFAIR", causality_type="chain_reaction", mechanism="epistemic", evidence_strength="moderate", causal_force=5.0, fabula_time=4),
        CausalEdge(source_id="EVT_AMY_DISCOVERS_AFFAIR", target_id="EVT_AMY_DISAPPEARS", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=5),
        CausalEdge(source_id="EVT_AMY_DISAPPEARS", target_id="EVT_NICK_SUSPECT", causality_type="chain_reaction", mechanism="epistemic", evidence_strength="moderate", causal_force=5.0, fabula_time=6),
        CausalEdge(source_id="EVT_AMY_ROBBED", target_id="EVT_AMY_SEEKS_DESI", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=9),
        CausalEdge(source_id="EVT_NICK_TV_INTERVIEW", target_id="EVT_AMY_WATCHES_INTERVIEW", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=11),
        CausalEdge(source_id="EVT_AMY_WATCHES_INTERVIEW", target_id="EVT_DESI_MURDERED", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=13),
        CausalEdge(source_id="EVT_DESI_MURDERED", target_id="EVT_AMY_RETURNS", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=14),
        CausalEdge(source_id="EVT_AMY_RETURNS", target_id="EVT_FORCED_MARRIAGE", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=15),
        CausalEdge(source_id="EVT_AMY_INSEMINATES", target_id="EVT_NICK_SUBMITS", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=17),
    ],
