...
world_state = WorldStateV1(
    ...
    events=[
        EventNode(id="EVT_WILLARD_IN_SAIGON", fabula_time=1, syuzhet_index=1, event_type="outcome", actor_ids=["ENT_WILLARD"], description="Willard is holed up in a Saigon hotel room, intoxicated and desperate for a mission, having returned for a second tour."),
        EventNode(id="EVT_MISSION_BRIEFING", fabula_time=2, syuzhet_index=2, event_type="choice", actor_ids=[], description="Willard is briefed at Nha Trang: terminate Colonel Kurtz, a rogue Green Beret commanding a Montagnard army in Cambodia."),
        EventNode(id="EVT_KILGORE_ATTACK", fabula_time=3, syuzhet_index=3, event_type="choice", actor_ids=["ENT_KILGORE"], description="Kilgore orders an air attack on a Vietcong village, playing Wagner's Ride of the Valkyries. The PBR is placed in the river."),
        EventNode(id="EVT_TIGER_ENCOUNTER", fabula_time=4, syuzhet_index=4, event_type="outcome", actor_ids=[], description="Chef and Willard disembark in the jungle searching for mangoes. A tiger lunges at them; Chef has a nervous breakdown."),
        EventNode(id="EVT_USO_SHOW", fabula_time=5, syuzhet_index=5, event_type="outcome", actor_ids=[], description="At a supply depot, the crew watches a USO show with Playboy Playmates that descends into chaos as soldiers storm the stage."),
        EventNode(id="EVT_SAMPAN_MASSACRE", fabula_time=6, syuzhet_index=6, event_type="choice", actor_ids=["ENT_WILLARD"], target_ids=["ENT_CHIEF"], description="Chief orders a sampan search. Clean panics and kills the civilians. Willard executes the surviving woman to avoid a detour."),
        EventNode(id="EVT_CLEAN_KILLED", fabula_time=7, syuzhet_index=7, event_type="outcome", actor_ids=[], description="Past Do Lung Bridge, the PBR comes under surprise Vietcong attack. Clean is fatally shot while listening to a tape from his mother."),
        EventNode(id="EVT_CHIEF_KILLED", fabula_time=8, syuzhet_index=8, event_type="outcome", actor_ids=[], description="Natives attack the PBR with arrows. Chief is impaled with a spear and dies."),
        EventNode(id="EVT_ARRIVE_COMPOUND", fabula_time=9, syuzhet_index=9, event_type="outcome", actor_ids=[], description="The surviving crew reaches Kurtz's macabre compound, strewn with corpses and severed heads. The photojournalist greets them."),
        EventNode(id="EVT_WILLARD_IMPRISONED", fabula_time=10, syuzhet_index=10, event_type="outcome", actor_ids=["ENT_KURTZ"], description="Kurtz imprisons Willard in a tiger cage. During the night, Kurtz throws Chef's severed head into Willard's lap."),
        EventNode(id="EVT_KURTZ_PHILOSOPHIZES", fabula_time=11, syuzhet_index=11, event_type="revelation", actor_ids=["ENT_KURTZ"], description="Willard is freed and given freedom to roam. He listens to Kurtz's philosophizing for several days."),
        EventNode(id="EVT_KURTZ_KILLED", fabula_time=12, syuzhet_index=12, event_type="choice", actor_ids=["ENT_WILLARD"], target_ids=["ENT_KURTZ"], description="Intercut with the ritual sacrifice of a caribou, Willard slaughters Kurtz with a machete. Kurtz's last words: 'The horror, the horror.'"),
        EventNode(id="EVT_WILLARD_DEPARTS", fabula_time=13, syuzhet_index=13, event_type="choice", actor_ids=["ENT_WILLARD"], description="The natives acknowledge Willard as their new leader. He throws down the machete, collects Lance, and departs on the PBR."),
    ],

    # ── CAUSAL TOPOLOGY ────────────────────────────────────────────────────
    causal_topology=[
        CausalEdge(source_id="EVT_WILLARD_IN_SAIGON", target_id="EVT_MISSION_BRIEFING", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=1),
        CausalEdge(source_id="EVT_MISSION_BRIEFING", target_id="EVT_KILGORE_ATTACK", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=2),
        CausalEdge(source_id="EVT_KILGORE_ATTACK", target_id="EVT_TIGER_ENCOUNTER", causality_type="chain_reaction", mechanism="physical", evidence_strength="moderate", causal_force=5.0, fabula_time=3),
        CausalEdge(source_id="EVT_TIGER_ENCOUNTER", target_id="EVT_USO_SHOW", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=4),
        CausalEdge(source_id="EVT_SAMPAN_MASSACRE", target_id="EVT_CLEAN_KILLED", causality_type="chain_reaction", mechanism="physical", evidence_strength="moderate", causal_force=5.0, fabula_time=6),
        CausalEdge(source_id="EVT_CLEAN_KILLED", target_id="EVT_CHIEF_KILLED", causality_type="chain_reaction", mechanism="physical", evidence_strength="moderate", causal_force=5.0, fabula_time=7),
        CausalEdge(source_id="EVT_CHIEF_KILLED", target_id="EVT_ARRIVE_COMPOUND", causality_type="chain_reaction", mechanism="physical", evidence_strength="moderate", causal_force=5.0, fabula_time=8),
        CausalEdge(source_id="EVT_ARRIVE_COMPOUND", target_id="EVT_WILLARD_IMPRISONED", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=9),
        CausalEdge(source_id="EVT_WILLARD_IMPRISONED", target_id="EVT_KURTZ_PHILOSOPHIZES", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=10),
        CausalEdge(source_id="EVT_KURTZ_PHILOSOPHIZES", target_id="EVT_KURTZ_KILLED", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=11),
        CausalEdge(source_id="EVT_KURTZ_KILLED", target_id="EVT_WILLARD_DEPARTS", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=12),
    ],
