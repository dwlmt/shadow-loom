...
world_state = WorldStateV1(
    ...
    events=[
        EventNode(id="EVT_HEIST", fabula_time=1, syuzhet_index=1, event_type="choice", actor_ids=["ENT_GEORGE"], description="George, Ken, Wanda, and Otto execute a successful jewel heist. Diamonds are hidden in a safe in the old workshop."),
        EventNode(id="EVT_WANDA_OTTO_BETRAY_GEORGE", fabula_time=2, syuzhet_index=2, event_type="choice", actor_ids=["ENT_WANDA"], description="Wanda and Otto betray George to the police. He is arrested."),
        EventNode(id="EVT_DIAMONDS_MOVED", fabula_time=3, syuzhet_index=3, event_type="outcome", actor_ids=["ENT_GEORGE"], description="George has already moved the diamonds. Wanda finds the safe deposit key in Ken's fish tank and hides it in her pendant."),
        EventNode(id="EVT_WANDA_SEDUCES_ARCHIE", fabula_time=4, syuzhet_index=4, event_type="choice", actor_ids=["ENT_WANDA"], description="Wanda seduces George's barrister Archie Leach to get him to persuade George to plead guilty and reveal the diamonds' location."),
        EventNode(id="EVT_ARCHIE_FALLS_FOR_WANDA", fabula_time=5, syuzhet_index=5, event_type="outcome", actor_ids=["ENT_ARCHIE"], description="Archie, in a loveless marriage, falls for Wanda. Otto's jealous interference causes their meetings to go wrong."),
        EventNode(id="EVT_PENDANT_LOST", fabula_time=6, syuzhet_index=6, event_type="outcome", actor_ids=["ENT_WANDA"], description="Wanda accidentally leaves her pendant at Archie's house. Wendy mistakes it for a gift. Archie stages a burglary to recover it."),
        EventNode(id="EVT_ARCHIE_ENDS_AFFAIR", fabula_time=7, syuzhet_index=7, event_type="choice", actor_ids=["ENT_ARCHIE"], description="Feeling guilty, Archie ends the affair with Wanda."),
        EventNode(id="EVT_KEN_KILLS_DOGS", fabula_time=8, syuzhet_index=8, event_type="outcome", actor_ids=["ENT_KEN"], target_ids=["ENT_MRS_COADY"], description="Ken's attempts to kill witness Mrs Coady accidentally kill her three dogs. She ultimately dies of a heart attack."),
        EventNode(id="EVT_WANDA_TESTIFIES_AGAINST_GEORGE", fabula_time=9, syuzhet_index=9, event_type="choice", actor_ids=["ENT_WANDA"], description="At trial, defence witness Wanda unexpectedly testifies against George."),
        EventNode(id="EVT_ARCHIE_EXPOSED", fabula_time=10, syuzhet_index=10, event_type="outcome", actor_ids=["ENT_ARCHIE"], description="Archie calls Wanda 'darling' during cross-examination. Wendy realizes the affair and decides to divorce him."),
        EventNode(id="EVT_OTTO_EATS_FISH", fabula_time=11, syuzhet_index=11, event_type="choice", actor_ids=["ENT_OTTO"], description="Otto eats Ken's pet fish one by one to force Ken to reveal the diamonds' location at the Heathrow hotel."),
        EventNode(id="EVT_ARCHIE_RESOLVES_STEAL", fabula_time=12, syuzhet_index=12, event_type="choice", actor_ids=["ENT_ARCHIE"], description="With career and marriage over, Archie resolves to steal the diamonds and flee to South America."),
        EventNode(id="EVT_DIAMOND_CHASE", fabula_time=13, syuzhet_index=13, event_type="outcome", actor_ids=[], description="Otto steals Archie's car with Wanda. They recover the diamonds, but Wanda double-crosses Otto and knocks him out."),
        EventNode(id="EVT_OTTO_VS_ARCHIE", fabula_time=14, syuzhet_index=14, event_type="outcome", actor_ids=["ENT_OTTO"], description="Otto escapes and confronts Archie. Archie stalls by taunting Otto about American failures."),
        EventNode(id="EVT_STEAMROLLER", fabula_time=15, syuzhet_index=15, event_type="outcome", actor_ids=["ENT_KEN"], description="Ken drives a steamroller over Otto (stuck in wet concrete) seeking vengeance for his fish. Otto survives."),
        EventNode(id="EVT_ESCAPE", fabula_time=16, syuzhet_index=16, event_type="outcome", actor_ids=["ENT_ARCHIE"], description="Archie and Wanda board the plane with the diamonds. Otto clings to the window until blown off during takeoff."),
    ],

    # ── CAUSAL TOPOLOGY ────────────────────────────────────────────────────
    causal_topology=[
        CausalEdge(source_id="EVT_HEIST", target_id="EVT_WANDA_OTTO_BETRAY_GEORGE", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=1),
        CausalEdge(source_id="EVT_WANDA_OTTO_BETRAY_GEORGE", target_id="EVT_DIAMONDS_MOVED", causality_type="chain_reaction", mechanism="epistemic", evidence_strength="moderate", causal_force=5.0, fabula_time=2),
        CausalEdge(source_id="EVT_WANDA_OTTO_BETRAY_GEORGE", target_id="EVT_WANDA_SEDUCES_ARCHIE", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=2),
        CausalEdge(source_id="EVT_WANDA_SEDUCES_ARCHIE", target_id="EVT_ARCHIE_FALLS_FOR_WANDA", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=4),
        CausalEdge(source_id="EVT_PENDANT_LOST", target_id="EVT_ARCHIE_ENDS_AFFAIR", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=6),
        CausalEdge(source_id="EVT_KEN_KILLS_DOGS", target_id="EVT_WANDA_TESTIFIES_AGAINST_GEORGE", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=8),
        CausalEdge(source_id="EVT_WANDA_TESTIFIES_AGAINST_GEORGE", target_id="EVT_ARCHIE_EXPOSED", causality_type="chain_reaction", mechanism="social", evidence_strength="moderate", causal_force=5.0, fabula_time=9),
        CausalEdge(source_id="EVT_OTTO_EATS_FISH", target_id="EVT_DIAMOND_CHASE", causality_type="chain_reaction", mechanism="epistemic", evidence_strength="moderate", causal_force=5.0, fabula_time=11),
        CausalEdge(source_id="EVT_ARCHIE_EXPOSED", target_id="EVT_ARCHIE_RESOLVES_STEAL", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=10),
        CausalEdge(source_id="EVT_DIAMOND_CHASE", target_id="EVT_OTTO_VS_ARCHIE", causality_type="chain_reaction", mechanism="physical", evidence_strength="moderate", causal_force=5.0, fabula_time=13),
        CausalEdge(source_id="EVT_OTTO_EATS_FISH", target_id="EVT_STEAMROLLER", causality_type="chain_reaction", mechanism="psychological", evidence_strength="moderate", causal_force=5.0, fabula_time=11),
    ],
